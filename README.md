# Hướng dẫn cấu hình I2C & giao tiếp EEPROM AT24CXX trên Raspberry Pi (Ubuntu)

Hướng dẫn từng bước cách bật I2C, cài đặt công cụ, và lập trình đọc/ghi module EEPROM AT24CXX trên Raspberry Pi chạy Ubuntu.
...
|---|---|
| `mem_addr >> 8` | **Address MSB** — dịch 8 bit sang phải, giữ lại phần địa chỉ cao |
| `mem_addr & 0xFF` | **Address LSB** — giữ lại 8 bit thấp, xóa phần cao |
...
1. **Chỉ đường (Write):** Pi gửi 2 byte địa chỉ để báo "tôi muốn đọc ô nhớ số X".
...
#### Tóm tắt cơ chế vật lý

```
Pi phát xung nhịp trên dây SCL
    → Pi gửi dữ liệu trên dây SDA (địa chỉ chip + địa chỉ ô nhớ)
        → Nếu GHI: chip im lặng và "nấu" dữ liệu vào bộ nhớ
        → Nếu ĐỌC: chip chủ động điều khiển SDA để gửi dữ liệu về Pi
```
